    ...
    ExecuteModule(module, parameters){
        try{
            switch module{
                ...
                case "BrowserDump":
                    parametersParts := StrSplit(parameters, ",")
                    includeChrome := (parametersParts.Length >= 1) ? parametersParts[1] : "true"
                    includeEdge := (parametersParts.Length >= 2) ? parametersParts[2] : "true"
                    includeFirefox := (parametersParts.Length >= 3) ? parametersParts[3] : "true"
                    this.BrowserDump(includeChrome, includeEdge, includeFirefox)
